...
import { Download, MapPin, Mail, Phone, Globe, Briefcase } from "lucide-react";
...
const aboutData = {
    ...
    highlights: [
        ...
        { icon: <Globe size={18} />, label: "Language", value: "Sinhala / English" },
        { icon: <Phone size={18} />, label: "Phone", value: "077-8638568" },
        { icon: <Mail size={18} />, label: "Email", value: "yohan.indrawansha2000@gmail.com" },
        { icon: <MapPin size={18} />, label: "Address", value: "No-178, Ullalapola, Divulapitya" },
    ],
    stats: [
                    ...
                    <div className="flex flex-col items-center lg:col-span-1">
                        ...
                        <div className="grid w-full max-w-sm grid-cols-3 gap-4 mt-8">
                            {aboutData.stats.map((stat, index) => (
                                <div key={index} className="p-3 text-center border rounded-lg bg-brand-gray border-brand-muted-gray/20">
                                    <div className="text-2xl font-bold text-brand-crimson-red">{stat.number}</div>
                                    <div className="mt-1 text-xs text-brand-muted-gray">{stat.label}</div>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-2">
                        ...
                        <div className="grid grid-cols-1 gap-4 mb-8 sm:grid-cols-2">
                            {aboutData.highlights.map((item, index) => (
                                <div
                                    key={index}
                                    className="flex items-center gap-3 p-4 transition-all duration-300 border rounded-lg border-brand-muted-gray/20 bg-brand-gray/50 hover:border-brand-crimson-red/40 hover:bg-brand-gray/80"
                                >
                        ...
                        <div className="mb-8">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {["UI/UX Design", "Frontend Dev", "Backend Dev", "Graphic Design"].map((skill) => (
                                    <span
                                        key={skill}
                                        className="px-4 py-2 text-sm font-medium transition-all duration-300 border rounded-full text-brand-light-gray border-brand-muted-gray/30 hover:border-brand-crimson-red hover:text-brand-crimson-red hover:bg-brand-crimson-red/5"
                                    >
                                        {skill}
                                    </span>
                                ))}
                        ...
                        <Link
                            ...
                            download
                            className="inline-flex items-center gap-2 px-8 py-3 font-semibold transition-all duration-300 rounded-lg text-brand-white bg-brand-crimson-red hover:bg-brand-soft-red hover:shadow-lg hover:shadow-brand-crimson-red/20 hover:scale-105"
                        >
                            <Download size={20} />
                            Download CV
                        </Link>
